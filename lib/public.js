...
module.exports = (shared) => {
  return {
     ...
     */
    global() {
      return shared.call(`/posts/streams/global`);
    },
     ...
     */
    user(userId) {
      return shared.call(`/users/${userId}`);
    },
    ...
    avatar(userId, buffered = true) {
      ...
      return shared.call(`/users/${userId}/avatar`, options);
    },
    ...
    cover(userId, buffered = true) {
      let options = buffered ? { resultAs: 'buffer' } : { resultAs: 'response' }
      return shared.call(`/users/${userId}/cover`, options);
    },
     ...
     */
    users(userIds) {
      return shared.call(`/users?ids=${userIds.join(',')}`);
    },
     ...
     */
    post(postId) {
      return shared.call(`/posts/${postId}`);
    },
     ...
     */
    posts(ids) {
      return shared.call(`/posts?ids=${ids.join(',')}`);
    },
     ...
     */
    revisions(postId) {
      return shared.call(`/posts/${postId}/revisions`);
    },
     ...
     */
    mentions(userId) {
      return shared.call(`/users/${userId}/mentions`);
    },
     ...
     */
    postsFrom(userId) {
      return shared.call(`/users/${userId}/posts`);
    },
     ...
     */
    postsTaggedWith(tag) {
      return shared.call(`/posts/tag/${encodeURI(tag)}`);
    },
     ...
     */
    thread(postId) {
      return shared.call(`/posts/${postId}/thread`);
    },
     ...
     */
    actions(postId) {
      return shared.call(`/posts/${postId}/actions`);
    }
  }
}
